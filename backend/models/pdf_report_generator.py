...
def create_report_pdf(
    ...
    Args:
        ...
        ai_report: Optional string containing the AI generated report
    Returns:
    ...
    if ai_report:
        ...
        splits = list(section_pattern.finditer(ai_report))
        section_spans = []
        for i, match in enumerate(splits):
            start = match.start()
            end = splits[i+1].start() if i+1 < len(splits) else len(ai_report)
            section_spans.append((match.group(1), ai_report[start+len(match.group(1)):end].strip()))
